...
const std::string LIB_PATH = getenv("HOWDY_LIB") == nullptr ? "" : getenv("HOWDY_LIB");
const std::string CONFIG_FILE_PATH = LIB_PATH + "/config.ini";
const std::string COMPARE_SCRIPT_PATH = LIB_PATH + "/compare.py";
...
auto identify(pam_handle_t *pamh, int flags, int argc, const char **argv,
  ...
  const char *const args[] = {PYTHON_EXECUTABLE, // NOLINT
                              COMPARE_SCRIPT_PATH.c_str(), username, nullptr};
  pid_t child_pid;
